...
public class Personal extends formPadre { // Hereda de su clase padre llamada formPadre

    // Constructor sobrecargado que recibe un String para establecer el nombre de la tabla
    public Personal(String Nom) {
        super("Tabla Personal");
        initComponents();
    }
    ...
    @Override
     public void Agregar(ActionEvent evt){
         if (evt.getSource() == btnAgregar) //Permite identificar que elemento del formulario disparó el evento
        {
            try
            {
                String strSQL = "INSERT INTO Personal (ID,Nombre) VALUES ('" + txtID.getText() + "','" + txtNombre.getText() + "')";
     ...
     public void Modificar(ActionEvent evt){
         if (evt.getSource() == btnModificar) //Lo mismo que el anterior
        {
            try
            {
                String strSQL = "UPDATE Personal SET Nombre = '" + txtNombre.getText() + "' where ID = '" + txtID.getText() + "'";
                Statement stmt = con.createStatement(); //Declaración de variable y preparación de sentencia SQL
     ...
     public void Consultar(ActionEvent evt){
         if (evt.getSource() == btnConsultar) //Varia un poco
        {
            try
            {
                String SQL = "SELECT * FROM Personal where ID ='" + txtID.getText() + "'"; //Texto de sentencia SQL (query)
                Statement stmt = con.createStatement(); //Declaración de variable y preparación de sentencia SQL
     ...
     // Lo mismo que arriba
      ...
      public void Borrar(ActionEvent evt){
            if (evt.getSource() == btnBorrar) //Lo mismo que el anterior
        {
            try
            {
                String strSQL = "DELETE from Personal where ID = '" + txtID.getText() + "'";
                Statement stmt = con.createStatement(); //Declaración de variable y preparación de sentencia SQL
                int rowsEffected = stmt.executeUpdate(strSQL);
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                JLabel lblPe = new JLabel("Tabla Personal");
                new Personal(null).setVisible(true);
            }
        });
    }
